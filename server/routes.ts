...
import { generateMindReport } from "./services/mindReport";
...
export async function registerRoutes(
  ...
  app.get("/api/watchlist", async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: err.message || "Failed to fetch watchlist" });
    }
  });
  ...
  app.get("/api/signals", async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: err.message || "Failed to fetch signals" });
    }
  });
  ...
  app.get("/api/trades", async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: err.message || "Failed to fetch trades" });
    }
  });
      ...
      if (parsed.emotionBefore === "revenge" || parsed.emotionBefore === "excited") {
        ...
        if (wss) {
          const alertMsg = JSON.stringify({
            type: "intervention",
            severity: "high",
            message: parsed.emotionBefore === "revenge"
              ? `🚨 Revenge trade detected on ${parsed.ticker}`
              : `⚡ FOMO entry detected on ${parsed.ticker}`,
  ...
  app.get("/api/behavioral-logs", async (_req, res) => {
    try {
      ...
      res.status(500).json({ message: err.message || "Failed to fetch behavioral logs" });
    }
  });
  ...
  app.get("/api/mind-report", async (_req, res) => {
    try {
      const report = await generateMindReport();
      res.json(report);
    } catch (err: any) {
